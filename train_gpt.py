...
def eval_val_mp_global_ttt(h,device,val_data,base_model,batch_seqs=32):
	rank=h.rank;world_size=h.world_size;seq_len=h.eval_seq_len;stride=h.eval_stride;total_tokens=val_data.val_tokens.numel()-1;ttt_chunk=h.ttt_chunk_tokens;context_size=seq_len-stride;window_starts=[ws for ws in range(0,total_tokens,stride)if ws+context_size<total_tokens];num_chunks=(total_tokens+ttt_chunk-1)//ttt_chunk;chunk_windows=[[]for _ in range(num_chunks)]
	for ws in window_starts:wlen=min(ws+seq_len,total_tokens)-ws;s=0 if ws==0 else context_size;scored_start=ws+s;ci=min(scored_start//ttt_chunk,num_chunks-1);chunk_windows[ci].append(ws)
	log(f"mp_ttt:start phases={h.mp_phases} epochs_per_phase={h.ttt_epochs_per_phase} chunks={num_chunks} ttt_lr={h.ttt_lr}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);ttt_params=[p for p in base_model.parameters()]
	for p in ttt_params:p.requires_grad_(True)
	optimizer=torch.optim.SGD(ttt_params,lr=h.ttt_lr,momentum=h.ttt_momentum);loss_sum=token_count=byte_count=None
	for phase in range(h.mp_phases):
		loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);base_model.eval()
		for ci in range(num_chunks):
			windows=chunk_windows[ci]
			if not windows:continue
			...
			with torch.no_grad():
				for bi in range(0,len(my_windows),batch_seqs):
					...
					nll=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),y_batch.reshape(-1),reduction='none').reshape(bsz,seq_len)
					for(i,ws)in enumerate(batch_ws):wlen=wlens[i];s=0 if ws==0 else context_size;scored_nll=nll[i,s:wlen].to(torch.float64);loss_sum+=scored_nll.sum();token_count+=float(wlen-s);tgt=y_batch[i,s:wlen];prev=x_batch[i,s:wlen];tb=val_data.base_bytes_lut[tgt].to(torch.float64);tb+=(val_data.has_leading_space_lut[tgt]&~val_data.is_boundary_token_lut[prev]).to(torch.float64);byte_count+=tb.sum()
		log(f"mp_ttt:phase {phase+1}/{h.mp_phases} score_done")
		if phase==h.mp_phases-1:break
		base_model.train()
		for ci in range(num_chunks):
			chunk_start=ci*ttt_chunk;chunk_end=min((ci+1)*ttt_chunk,total_tokens);chunk_seqs=(chunk_end-chunk_start)//seq_len
			if chunk_seqs<=0:continue
			cos_lr=h.ttt_lr*.5*(1.+math.cos(math.pi*ci/max(num_chunks-1,1)))
			for pg in optimizer.param_groups:pg['lr']=cos_lr
			my_seq_s=chunk_seqs*rank//world_size;my_seq_e=chunk_seqs*(rank+1)//world_size;my_chunk_seqs=my_seq_e-my_seq_s
			for _ep in range(h.ttt_epochs_per_phase):
				for bs in range(0,my_chunk_seqs,batch_seqs):
					be=min(bs+batch_seqs,my_chunk_seqs);actual_bs=my_seq_s+bs;start_tok=chunk_start+actual_bs*seq_len;end_tok=chunk_start+(my_seq_s+be)*seq_len+1
					if end_tok>val_data.val_tokens.numel():continue
					local=val_data.val_tokens[start_tok:end_tok].to(device=device,dtype=torch.int64);x=local[:-1].reshape(-1,seq_len);y=local[1:].reshape(-1,seq_len);optimizer.zero_grad(set_to_none=True)
...
def train_model(h,device,val_data):
	...
	swa_state={name:torch.zeros_like(t.detach().float())for(name,t)in base_model.state_dict().items()}if h.swa_enabled else None;swa_count=0;swa_threshold_ms=(1e3*h.max_wallclock_seconds if h.max_wallclock_seconds>0 else 0)*(1.-h.swa_window_frac)
	while True:
		...
		if h.swa_enabled and step%h.swa_interval==0 and approx_training_time_ms>=swa_threshold_ms:
			with torch.no_grad():
	...
	if h.swa_enabled and swa_count>0:
		log(f"swa:blending {swa_count} snapshots into EMA shadow (blend={h.swa_blend:.2f}, window_frac={h.swa_window_frac:.3f})")
		for name in ema_state:swa_avg=swa_state[name]/swa_count;ema_state[name].mul_(1.-h.swa_blend).add_(swa_avg,alpha=h.swa_blend)
	elif h.swa_enabled:
		log(f"swa:no snapshots collected (window not entered before training ended; window_frac={h.swa_window_frac:.3f}, interval={h.swa_interval})")
	log('ema:applying EMA weights');current_state=base_model.state_dict();avg_state={name:t.to(dtype=current_state[name].dtype)for(name,t)in ema_state.items()};base_model.load_state_dict(avg_state,strict=True);return base_model,compiled_model
def train_and_eval(h,device):
	...
	if h.ttt_enabled and h.sliding_window_enabled:
		...
		if h.mp_ttt_enabled:timed_eval('quantized_mp_global_ttt',eval_val_mp_global_ttt,h,device,val_data,ttt_model)
		else:timed_eval('quantized_ttt',eval_val_ttt,h,device,val_data,ttt_model)
		del ttt_model
