...
class Optimizers:
	def __init__(self,h,base_model):
		block_named_params=list(base_model.blocks.named_parameters());matrix_named=[(name,p)for(name,p)in block_named_params if p.ndim==2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)];matrix_params=[p for(_,p)in matrix_named];scalar_params=[p for(name,p)in block_named_params if p.ndim<2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)]
		...
		mlp_params=[p for(name,p)in matrix_named if'mlp.'in name];attn_params=[p for(name,p)in matrix_named if'attn.'in name];other_matrix_params=[p for(name,p)in matrix_named if'mlp.'not in name and'attn.'not in name]
		muon_groups=[{'params':mlp_params,'weight_decay':h.mlp_wd}if mlp_params else None,{'params':attn_params,'weight_decay':h.attn_wd}if attn_params else None,{'params':other_matrix_params,'weight_decay':h.muon_wd}if other_matrix_params else None];muon_groups=[g for g in muon_groups if g is not None]
		token_lr=h.tied_embed_lr if h.tie_embeddings else h.embed_lr;tok_params=[{'params':[base_model.tok_emb.weight],'lr':token_lr,'base_lr':token_lr}];self.optimizer_tok=torch.optim.AdamW(tok_params,betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.embed_wd,fused=True);self.optimizer_muon=Muon(muon_groups,lr=h.matrix_lr,momentum=h.muon_momentum,backend_steps=h.muon_backend_steps,weight_decay=h.muon_wd,row_normalize=h.muon_row_normalize)
